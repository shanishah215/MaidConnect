...
class _RequestManagementPageState extends State<RequestManagementPage> with SingleTickerProviderStateMixin {
  ...
  void _updateDisplay() {
    ...
    setState(() {
      _displayInquiries = AdminPortalStore.instance.getInquiriesByStatus(status);
      if (_searchController.text.isNotEmpty) {
        final query = _searchController.text.toLowerCase();
        _displayInquiries = _displayInquiries.where((i) =>
          i.clientName.toLowerCase().contains(query) ||
          i.maidName.toLowerCase().contains(query)).toList();
      }
    });
  }
  ...
  InquiryStatus? _getStatusFromTabIndex(int index) {
    switch (index) {
      case 1: return InquiryStatus.pending;
      case 2: return InquiryStatus.approved;
      case 3: return InquiryStatus.rejected;
      case 4: return InquiryStatus.assigned;
      default: return null;
    }
  }
  ...
  Future<void> _approve(ClientInquiry inq) async {
    final ok = await AdminDialog.confirm(
      ...
      message: 'Approve request from ${inq.clientName} for ${inq.maidName}?',
  ...
  Future<void> _assign(ClientInquiry inq) async {
    ...
    if (maids.isEmpty) {
      AdminDialog.showSnack(context, 'No available maids to assign.', isError: true);
      return;
    }
            ...
            child: Container(
              ...
              child: _isLoading
                  ...
                  : _displayInquiries.isEmpty
                      ? const Center(child: Text('No inquiries found in this category.'))
                      : SingleChildScrollView(
                          child: DataTable(
                            horizontalMargin: 24,
                            columnSpacing: 24,
                            columns: const [
                              DataColumn(label: Text('Inquiry ID', style: TextStyle(fontWeight: FontWeight.w700))),
                              DataColumn(label: Text('Client', style: TextStyle(fontWeight: FontWeight.w700))),
                              DataColumn(label: Text('Maid Preferred', style: TextStyle(fontWeight: FontWeight.w700))),
                            ...
                            rows: _displayInquiries.map((inq) {
                              return DataRow(cells: [
                                DataCell(Text(inq.id, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13))),
                                ...
                                DataCell(Text(inq.assignedTo ?? '-', style: TextStyle(color: inq.assignedTo != null ? const Color(0xFF6366F1) : Colors.black))),
                                DataCell(Row(
                                  children: [
                                    ...
                                    if (inq.status == InquiryStatus.approved)
                                      IconButton(
                                        icon: const Icon(Icons.assignment_ind_outlined, color: Color(0xFF6366F1), size: 20),
                                        onPressed: () => _assign(inq),
                                        tooltip: 'Assign Maid',
                                      ),
                                    IconButton(
                                      icon: const Icon(Icons.visibility_outlined, size: 20, color: Color(0xFF64748B)),
                                      onPressed: () => AdminDialog.showSnack(context, 'Mock: Inq note: ${inq.notes}'),
                                      tooltip: 'View Notes',
                                    ),
                                  ],
                                )),
